...
func (r *mutationResolver) Register(ctx context.Context, input model.RegisterInput) (*model.AuthPayload, error) {
	...
	deviceInfo := &authpb.DeviceInfo{
		...
		LocationCountry:   *input.DeviceInfo.LocationCountry,
		LocationCity:      *input.DeviceInfo.LocationCity,
		Latitude:          *input.DeviceInfo.Latitude,
		Longitude:         *input.DeviceInfo.Longitude,
	}
...
// Soc2Overview is the resolver for the soc2Overview field.
func (r *queryResolver) Soc2Overview(ctx context.Context) (*model.SOC2Overview, error) {

	// 1️⃣ Extract authenticated user context (FAIL CLOSED)
	...
	allowed := r.Clients.CheckPermission(ctx, userCtx.UserID, orgID, "view_compliance_dashboard")
	if !allowed {
		return nil, fmt.Errorf("forbidden: insufficient permissions to view compliance dashboard")
	}

	// 3️⃣ Call Compliance gRPC (Audit Service)
	resp, err := r.Clients.ComplianceClient.GetSOC2Overview(
		ctx,
		&compb.SOC2Request{
	...
	// 4️⃣ Map gRPC → GraphQL model
	return &model.SOC2Overview{
		OrganizationID: resp.OrganizationId,
		AccessControl: &model.AccessControlMetrics{
			TotalUsers: int(resp.AccessControl.TotalUsers),
			AdminUsers: int(resp.AccessControl.AdminUsers),
		},
		Authentication: &model.AuthenticationMetrics{
			MfaEnabledPercentage: resp.Authentication.MfaEnabledPercentage,
			FailedLogins30Days:   int(resp.Authentication.FailedLogins_30Days),
			AuthzDenials30Days:   int(resp.Authentication.AuthzDenials_30Days),
		},
		SystemOperations: &model.SystemOperationsMetrics{
			ActiveSessions:   int(resp.SystemOperations.ActiveSessions),
			UnresolvedAlerts: int(resp.SystemOperations.UnresolvedAlerts),
		},
		SecurityIncidents: &model.SecurityIncidentMetrics{
			HighSeverityAlerts:   int(resp.SecurityIncidents.HighSeverityAlerts),
			MediumSeverityAlerts: int(resp.SecurityIncidents.MediumSeverityAlerts),
		},
	}, nil
}
